...
bool irreducible(const polynomial_mpz * restrict f, const unsigned long p)
{
    ...
    for (size_t i = 0 ; i <= f->degree>>1 ; i++)
    {
        ...
        power_poly_mod(&tmp_poly, &g, f, p, p);
...
void poly_div(polynomial_mpz * restrict res, const polynomial_mpz *f, const polynomial_mpz *g) // Returns remainder polynomial only, not quotient
{
    reset_polynomial(res);
...
void poly_div_mod(polynomial_mpz * restrict res, const polynomial_mpz *f, const polynomial_mpz *g, const unsigned long p) // Returns remainder polynomial only, not quotient
{
    reset_polynomial(res);
...
void poly_div_mod_mpz(polynomial_mpz * restrict res, const polynomial_mpz *f, const polynomial_mpz *g, const mpz_t p) // Returns remainder polynomial only, not quotient
{
    reset_polynomial(res);
    ...
    if (f->degree >= g->degree)
    {
        ...
        for (size_t i = 0 ; i <= f->degree ; i++)
        {
            mpz_mod(tmp, f->coeffs[i], p);
            set_coeff(&remainder, tmp, f->degree - i); // Same as poly prod, set in reversal order to allocate only once
        }

        for (size_t i = 0 ; i <= g->degree ; i++)
        {
            mpz_mod(tmp, g->coeffs[i], p);
            set_coeff(&reduced_g, tmp, g->degree - i);
        }
        ...
        for (size_t i = 0 ; i <= f->degree - reduced_g.degree ; i++)
        {
            if (mpz_cmp_ui(remainder.coeffs[i], 0))
            {
                ...
                {
                    mpz_addmul(remainder.coeffs[i + j], tmp, reduced_g.coeffs[j]);
                    mpz_mod(remainder.coeffs[i + j], remainder.coeffs[i + j], p);
                }
            }
        }
...
void quotient_poly_mod(polynomial_mpz * restrict res, const polynomial_mpz *f, const polynomial_mpz *g, const unsigned long p) // Returns quotient polynomial only, not remainder
{
    reset_polynomial(res);
